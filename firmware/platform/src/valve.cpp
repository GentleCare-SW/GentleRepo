...
Valve::Valve(const char *proportional_uuid, const char *on_off_uuid, int32_t digital_pin, int32_t pwm_pin, int32_t ledc_channel)
{
    ...
    this->add_characteristic(proportional_uuid, std::bind(&Valve::set_voltage, this, std::placeholders::_1), std::bind(&Valve::get_voltage, this));
...
void Valve::set_voltage(float voltage)
{
    this->voltage = constrain(voltage, 0.0, 15.0);
    uint32_t duty_cycle = this->voltage / 15.0 * 0xffff;
    ledcWrite(this->ledc_channel, duty_cycle);
}

float Valve::get_voltage()
{
    return this->voltage;
}
